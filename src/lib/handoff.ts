...
/**
 * `secureCookies` AND `sessionSameSite` NOW LIVE IN `lib/cookieFlags.ts`, and
 * are re-exported here so that `src/auth.ts`, the redeem route, the keepalive
 * route and `origin.check.ts` keep the import they already have.
 *
 * THEY MOVED BECAUSE THE BROWSER NEEDS THEM. `lib/prefs.ts` writes the display
 * preferences with `document.cookie` from inside a `'use client'` boundary, and
 * this file imports `node:crypto` and the DynamoDB client — so it can never be
 * reached from there. The copy of the rule that grew up next to that
 * `document.cookie` said `samesite=lax`, which is not sent in a third-party
 * context, which is why the framed console re-offered first-run setup on every
 * navigation. The reasoning for `None`, unchanged, is in the new file.
 */
export { secureCookies, sessionSameSite } from './cookieFlags'
